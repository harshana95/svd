...
def get_pca_components(psfs_all, pca_n=100):
    ...
    weights = np.zeros((inc, outc, pca_n + 1, h, w), dtype='float32')
    ...
    for i in range(inc):
        for j in range(outc):
            ...
            weights[i, j, :pca_n] = psf_c_transformed.reshape((h, w, pca_n)).transpose([2, 0, 1])
            weights[i, j, pca_n] = 1.0  # always add mean
    ...
    return basis_psfs, weights, pca_components, pca_mean, pca_var
