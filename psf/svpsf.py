...
class PSFSimulator:
    ...
    def generate_psf(self, x, y, ch, **kwargs):
        ...
        if psf_type == 'gaussian':
            ...
            weights = kwargs.get('basis_weights')
    ...
    def save_psfs(path, psfs,
                  ...
                  basis_psfs=None, basis_weights=None,
        ...
        if basis_weights is not None:
            print(f"basis_weights {basis_weights.shape}")
            hf.create_dataset("basis_weights", data=basis_weights.astype(dtype))
        ...
        if save_as_image:
            ...
            if basis_weights is not None:
                generate_folder(join(path, 'basis_weights'))
            ...
            for i in range(basis_psfs.shape[0]):
                for j in range(basis_psfs.shape[1]):
                    for n in range(basis_psfs.shape[2]):
                        ...
                        if basis_weights is not None:
                            cv2.imwrite(join(path, 'basis_weights', f'{i}_{j}_{n}.png'),
                                        128 + (basis_weights[i, j, n] * 255).astype(int))
